do
	...
	---@param angle real radian
	---@return real, real
	function GetPolarOffsetXY(x, y, distance, angle)
		return distance * math.cos(angle) + x, distance * math.sin(angle) + y
	end

	local GetTerrainZ_location = 0
	---@param x real
	---@param y real
	---@return real
	function GetTerrainZ(x, y)
		if GetTerrainZ_location == 0 then GetTerrainZ_location = Location(0, 0) end
	...
	local SetUnitZ_ability = FourCC('Aave')
	---@param target unit
	---@param z real
	function SetUnitZ(target, z)
		UnitAddAbility(target, SetUnitZ_ability)
		UnitRemoveAbility(target, SetUnitZ_ability)
		MoveLocation(GetTerrainZ_location, GetUnitX(target), GetUnitY(target))
