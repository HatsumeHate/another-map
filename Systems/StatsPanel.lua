do
    ...
    local function Update()
        for i = 1, 6 do
            if PlayerHero[i] ~= nil then
                ...
                BlzFrameSetText(StatsList[PHYSICAL_ATTACK], "Физ. урон: ".. R2I(data.stats[PHYSICAL_ATTACK].value))
                BlzFrameSetText(StatsList[PHYSICAL_DEFENCE], "Защита: ".. R2I(data.stats[PHYSICAL_DEFENCE].value))
                BlzFrameSetText(StatsList[MAGICAL_ATTACK], "Маг. урон: ".. R2I(data.stats[MAGICAL_ATTACK].value))
    ...
    local function StatButtonClick()
        ...
        ModifyStat(PlayerHero[id], MainStatButtons[id].frames[GetHandleId(button)].stat, 1, STRAIGHT_BONUS, true)
    end
    ...
    local function DrawInterface(player)
        ...
        MainStatButtons[player].frames[GetHandleId(button)].button_frame = button
        MainStatButtons[player].frames[GetHandleId(button)].stat = INT_STAT
        ...
        MainStatButtons[player].frames[GetHandleId(button)] = {}
        MainStatButtons[player].frames[GetHandleId(button)].button_frame = button
        MainStatButtons[player].frames[GetHandleId(button)].stat = VIT_STAT
        BlzTriggerRegisterFrameEvent(trg, button, FRAMEEVENT_CONTROL_CLICK)
        ...
        MainStatButtons[player].frames[GetHandleId(button)] = {}
        MainStatButtons[player].frames[GetHandleId(button)].button_frame = button
        MainStatButtons[player].frames[GetHandleId(button)].stat = AGI_STAT
        BlzTriggerRegisterFrameEvent(trg, button, FRAMEEVENT_CONTROL_CLICK)
        ...
        MainStatButtons[player].frames[GetHandleId(button)] = {}
        MainStatButtons[player].frames[GetHandleId(button)].button_frame = button
        MainStatButtons[player].frames[GetHandleId(button)].stat = STR_STAT
        BlzTriggerRegisterFrameEvent(trg, button, FRAMEEVENT_CONTROL_CLICK)
